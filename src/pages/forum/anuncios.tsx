    ...
    <div className="absolute right-0 top-12 mt-2 w-64 bg-[#1f2227] text-white rounded-lg shadow-lg">
      <div className="p-4">
        ...
        <ul className="space-y-1 mt-2">
          <li className="p-2 border-b border-[#444] flex items-center space-x-2">
            <div className="w-2 h-2 bg-[#4caf50] rounded-full"></div>
            <div>
              <p className="text-sm">
                Novo anúncio:{" "}
                <a href="#" className="text-[#1e90ff]">
                  Como se tornar um dev fudido
                </a>
              </p>
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-[#1f2227] text-white bg-opacity-50 z-50">
        <div className="relative bg-[#1f2227] p-6 rounded-lg shadow-lg max-w-sm w-full">
          <p className="text-red-500">Sem permissão para posts nesse tópico.</p>
          <button
            onClick={onClose}
...
export default function Anuncios() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        const response = await fetch("/api/posts?tag=anuncios", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="h-screen flex flex-col">
      <header className="flex items-center shadow-sm bg-[#262d34] font-[sans-serif] min-h-[70px] relative">
